...
rule consensusvcf:
    input:
        expand("results/mutect2/final/{pair_barcode}.final.dropgeno.vcf.gz", pair_barcode = manifest.getSelectedPairs())
    output:
    ...
    shell:
        "bcftools merge -Oz -o {output.vcf} {input} > {log} 2>&1;"
        "bcftools index -t {output.vcf} {input} > {log} 2>&1;"
