...
sort_df <-
  ...
  left_join(clindata) %>% 
  left_join(anpldata) %>%
  arrange(is_hypermutator, snv_driver_stability, cnv_driver_stability, snv_driver_evolution, snv_driver_count + cnv_driver_count, desc(mf_b)) # snv_driver_count, cnv_driver_count)#, aneuploidy_b - aneuploidy_a, 
...
plot_theme    <- theme_bw(base_size = 10) + theme(axis.title = element_text(size = 10), axis.text = element_text(size=10), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"))
...
gg_driver_context <-
  driv_hm %>% 
  gather(key = "type", value = "value", snv_driver_context_shared, snv_driver_context_change, cnv_driver_context_shared, cnv_driver_context_change, snv_driver_evolution, snv_driver_stability, cnv_driver_stability) %>%
  mutate(type = factor(type,
                       levels = c("cnv_driver_context_shared", "cnv_driver_context_change", "snv_driver_context_shared", "snv_driver_context_change", "snv_driver_evolution", "cnv_driver_stability", "snv_driver_stability"),
                       labels = c("Shared context - CNV", "Private context - CNV", "Shared context - SNV", "Private context - SNV", "Convergence - SNV", "Driver stability - CNV", "Driver stability - SNV"))) %>%
  ggplot(aes(x=case_barcode)) +
  geom_tile(aes(fill = value, y = type)) +
  scale_fill_manual(values=c("#ff7f00", "#ffff33", "#a65628", "#f781bf", "#999999"), na.value = "gray95") +
  labs(y="")
...
g6 <- ggplotGrob(gg_driver_context + plot_grid + plot_theme + null_legend + null_x + null_facet + bottom_margin) %>% gtable_frame()
g7 <- ggplotGrob(gg_mut_freq_prop_case + plot_grid + plot_theme + null_legend + null_x + null_facet + bottom_margin) %>% gtable_frame()
g8 <- ggplotGrob(gg_mut_heatmap + plot_grid + plot_theme + null_legend + null_x + null_facet + bottom_margin) %>% gtable_frame()
...
g8b <- ggplotGrob(gg_snv_prop_gene + plot_grid + plot_theme + null_legend + null_x + null_y + null_facet + bottom_margin) %>% gtable_frame()
