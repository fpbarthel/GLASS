...
q = "SELECT mf.tumor_pair_barcode, mf.fraction, mf.signature, mf.signature_score, mf.mut_count, ctp.hypermutator_status, ctp.received_tmz, tp.idh_codel_subtype 
...
nhm_kruskal_table <- matrix(0,ncol=length(subtypes),nrow=30)
colnames(nhm_kruskal_table) = subtypes
rownames(nhm_kruskal_table) <- as.character(1:30)
n = rep(0,3)
for(i in 1:length(subtypes))
{
	for(j in 1:30)
	{
		...
		nhm_kruskal_table[j,i] <- kruskal.test(signature_score~fraction,data=nhm_mutsig_table_sub)$p.value
...
g1 <- mean(nhm_mutsig_table[which(nhm_mutsig_table[,"fraction"]=="S" & nhm_mutsig_table[,"signature"]=="1"),"signature_score"])		#0.62
g2 <- mean(nhm_mutsig_table[which(nhm_mutsig_table[,"fraction"]=="P" & nhm_mutsig_table[,"signature"]=="1"),"signature_score"])		#0.36
g3 <- mean(nhm_mutsig_table[which(nhm_mutsig_table[,"fraction"]=="R" & nhm_mutsig_table[,"signature"]=="1"),"signature_score"])		#0.25

source("/projects/varnf/SofWar/R/geom_uperrorbar.r")
#Plot contribution barplot
pdf("/projects/varnf/GLASS/Figures/signatures/finals/barplots_nhm.pdf",width=7,height=1.8)
...
length(unique(hm_mutsig_table[,"tumor_pair_barcode"]))	#n = 22
...
for(i in 1:length(subtypes))
{
	for(j in 1:30)
	{
		...
		hm_kruskal_table[j,i] <- kruskal.test(signature_score~fraction,data=hm_mutsig_table_sub)$p.value
...
#Plot contribution barplot
pdf("/projects/varnf/GLASS/Figures/signatures/finals/barplots_hm.pdf",width=7,height=1.8)
