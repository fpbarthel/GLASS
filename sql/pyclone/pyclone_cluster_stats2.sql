...
pyclone_clusters AS
(
    SELECT pc.aliquot_barcode, sa.case_barcode, pc.cluster_id, sample_type, idh_codel_subtype, pc.size, pc.mean, pc.std, num_drivers
	FROM variants.pyclone_cluster pc
	INNER JOIN selected_aliquots sa ON sa.aliquot_barcode = pc.aliquot_barcode
	LEFT JOIN driver_counts t0 ON t0.case_barcode = sa.case_barcode AND t0.cluster_id = pc.cluster_id
	WHERE pc.size > 1
)/*,
t2 AS
(
