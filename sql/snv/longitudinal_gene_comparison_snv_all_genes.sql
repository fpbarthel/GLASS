...
selected_genes AS
(
	SELECT sn.gene_symbol, variant_id, chrom, pos, alt, sn.variant_classification, variant_classification_priority, protein_change
...
(
		SELECT DISTINCT gene_symbol FROM selected_genes
),
...
(
	SELECT gc.aliquot_barcode, gc.case_barcode, gc.gene_symbol, gene_cov
	FROM analysis.gene_coverage gc
	INNER JOIN selected_aliquots sa ON sa.aliquot_barcode = gc.aliquot_barcode
	INNER JOIN selected_genes_uq sg ON sg.gene_symbol = gc.gene_symbol
	WHERE sg.gene_symbol NOT IN ('IDH1','TERT')

	UNION ALL

	SELECT hc.*
	FROM hotspot_coverage hc
	INNER JOIN selected_aliquots sa ON sa.aliquot_barcode = hc.aliquot_barcode
),
...
(
	...
	--RIGHT JOIN gene_sample_coverage c1 ON c1.aliquot_barcode = gtc.tumor_barcode_a AND c1.gene_symbol = gtc.gene_symbol
	--LEFT JOIN gene_sample_coverage c2 ON c2.aliquot_barcode = gtc.tumor_barcode_b AND c2.gene_symbol = gtc.gene_symbol
	WHERE
...
(
	SELECT
		...
		gc.cov_a,
		gc.cov_b
	FROM gene_pair_coverage gc
	LEFT JOIN variants_by_case_and_gene vcg ON gc.tumor_barcode_a = vcg.tumor_barcode_a AND gc.tumor_barcode_b = vcg.tumor_barcode_b AND gc.gene_symbol = vcg.gene_symbol AND priority = 1
)
...
--SELECT * FROM gene_pair_coverage
--SELECT * FROM variants_by_case_and_gene LIMIT 100 --gene_sample_coverage
