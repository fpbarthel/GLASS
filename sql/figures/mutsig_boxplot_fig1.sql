WITH t1 AS
(
	SELECT gs.case_barcode, idh_codel_subtype, hypermutator_status::integer, fraction, signature, mut_n, abs_score, rel_score, RANK() OVER (PARTITION BY gs.case_barcode,fraction ORDER BY rel_score DESC) AS rnk, COUNT(*) OVER (PARTITION BY gs.case_barcode, signature) AS all_fractions_counts
