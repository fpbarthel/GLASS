...
selected_hypermut_groups AS
(
	SELECT DISTINCT hypermutator_status FROM analysis.tumor_clinical_comparison
),
variant_contexts AS
(
...
(
	...
	CROSS JOIN selected_hypermut_groups
),
...
(
	SELECT vca.*, COALESCE(mut_n,0) AS mut_n, SUM(COALESCE(mut_n,0)) OVER (PARTITION BY vca.fraction, vca.idh_codel_subtype, vca.hypermutator_status) AS mut_n_total
	FROM variant_contexts_aliquots vca
	LEFT JOIN variant_context_counts vcc ON vcc.fraction = vca.fraction AND vcc.idh_codel_subtype = vca.idh_codel_subtype AND vcc.hypermutator_status = vca.hypermutator_status AND vcc.trinucleotide_context = vca.trinucleotide_context AND vcc.alt = vca.alt
),
...
(
	SELECT fraction,idh_codel_subtype,hypermutator_status,ref_context_arr,sum(mut_n) AS mut_n, array_agg(mut_n ORDER BY trinucleotide_context,alt), lsqnonneg(ref_context_arr, array_agg(mut_n ORDER BY trinucleotide_context,alt)) AS mut_sigs
